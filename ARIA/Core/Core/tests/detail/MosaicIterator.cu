...
TEST(MosaicIterator, Mosaic) {
  ...
  // `int, float`.
  {
...

    {
      ...
      static_assert(std::is_same_v<decltype(*begin)::TMosaic, TMosaic>);
      static_assert(std::is_same_v<decltype(*end)::TMosaic, TMosaic>);
      static_assert(std::is_same_v<decltype(*beginC)::TMosaic, TMosaic>);
